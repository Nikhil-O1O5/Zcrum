...
export async function createSprint(projectId, data) {
  ...
  return sprint; // ✅ Ensure function returns the created sprint
}
...
export async function updateSprintStatus(sprintId, newStatus) {
  ...
  try {
    const sprint = await db.sprint.findUnique({
      ...
      include: {
        project: true, // ✅ Ensure project data is included for orgId check
      },
    });
    ...
    if (sprint.project.organizationId !== orgId) { // ✅ Fixed incorrect check
      throw new Error("Unauthorized");
    }
