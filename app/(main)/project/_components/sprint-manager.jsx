...
const SprintManager = ({ sprint, setSprint, sprints, projectId }) => {
  ...
  const handleSprintChange = (value) => {
    const selectedSprint = sprints.find((s) => s.id.toString() === value);
    if (selectedSprint) {
  ...
  return (
    <>
      <div className="flex justify-between items-center gap-4">
        <Select value={sprint.id.toString()} onValueChange={handleSprintChange}>
        ...
        {canStart && <Button className="bg-green-900 text-white"
         onClick={()=>handleStatusChange("ACTIVE")} disabled={loading}  >Start Sprint</Button>}
        {canEnd && <Button variant="destructive" onClick={()=>handleStatusChange("COMPLETED")} disabled={loading} >End Sprint</Button>}
      </div>
      {loading && <BarLoader width={"100%"} className="mt-2" color="#36d7b7" ></BarLoader>}
