...
export default function SprintCreationForm({
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
    setValue, // ✅ Used to update form values
  } = useForm({
  ...
  const onSubmit = async (data) => {
    console.log("📤 Sending Sprint Data:", {
      projectId,
      ...data,
      startDate: dateRange.from,
      endDate: dateRange.to,
    ...
    try {
      const response = await createSprintFn({
        projectId,
        ...data,
        startDate: dateRange.from,
        endDate: dateRange.to,
      ...
      if (!response || response.error) {
        console.error("🚨 Error:", response?.error || "Sprint creation failed");
        return;
      }
                        ...
                        <DayPicker
                          ...
                          onSelect={(range) => {
                            if (range?.from && range?.to) {
                              setDateRange(range);
                              setValue("startDate", range.from); // ✅ Ensure form value updates
